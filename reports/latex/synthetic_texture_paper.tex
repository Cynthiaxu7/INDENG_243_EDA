...
\begin{tikzpicture}[
  ...
  block/.style={draw, rounded corners=1pt, align=center, inner sep=4pt, minimum height=9mm},
  data/.style ={block, fill=blue!5, minimum width=28mm},
  model/.style={block, fill=orange!10, minimum width=32mm},
  outblk/.style={block, fill=green!8, minimum width=75mm},
  label/.style={font=\footnotesize\itshape, inner sep=1pt},
  arr/.style  ={->, shorten >=1pt, shorten <=1pt},
]
% input row
\node[data] (vid)  {Chewing videos\\ 20 subjects};
\node[data, right=of vid] (lm)  {MediaPipe face\\ landmarker};
\node[data, right=of lm]  (win) {Per-window features\\ 19 dim $\times$ ${\approx}8077$ windows};
\draw[arr] (vid) -- (lm);
\draw[arr] (lm)  -- (win);

% split labels
\node[label, below right=2mm and -14mm of win] (splt) {split on per-window vs.\ per-video};

% two paths
\node[model, below left=10mm and -6mm of win]  (tab) {Tabular baselines\\ \scriptsize LogReg\,$\cdot$\,RF\,$\cdot$\,XGB\,$\cdot$\,MLP\,$\cdot$\,FT-T\\ \scriptsize each window treated independently};
\node[model, below right=10mm and -6mm of win] (seq) {Bidirectional Mamba-1 \textbf{(ours)}\\ \scriptsize sort windows by \texttt{window\_id}\\ \scriptsize per video, selective SSM scan};

\draw[arr] (win.south) -- ++(0,-3mm) -| (tab.north);
\draw[arr] (win.south) -- ++(0,-3mm) -| (seq.north);

% converge to output block
\node[outblk, below=16mm of win] (outb) {Per-window logits $\to$ \{soft, medium, hard\}\\
...
\draw[arr] (tab.south) |- (outb.west);
\draw[arr] (seq.south) |- (outb.east);
